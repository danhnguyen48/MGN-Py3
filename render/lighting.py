...
def _get_face_normals(vertices, faces):
    ...
    vertices_by_index = tf.transpose(vertices, [vertices_ndim - 2] + range(
        vertices_ndim - 2) + [vertices_ndim - 1])  # indexed by vertex-index, *, x/y/z
    # indexed by face-index, vertex-in-face, *, x/y/z
    vertices_by_face = tf.gather(vertices_by_index, faces)
    normals_by_face = tf.cross(vertices_by_face[:, 1] - vertices_by_face[:, 0],
                               vertices_by_face[:, 2] - vertices_by_face[:, 0])  # indexed by face-index, *, x/y/z
    normals_by_face /= (tf.norm(normals_by_face, axis=-1,
                                keepdims=True) + 1.e-12)  # ditto
...
def vertex_normals_pre_split(vertices, faces, name=None, static=False):
    ...
    with ops.name_scope(name, 'VertexNormalsPreSplit', [vertices, faces]) as scope:
        ...
        vertices, faces = _prepare_vertices_and_faces(vertices, faces)
        vertices = vertices[..., :3]  # drop the w-coordinate if present
        face_count = int(faces.get_shape()[
                         0]) if static else tf.shape(faces)[0]

        normals_by_face, _ = _get_face_normals(
            vertices, faces)  # indexed by face-index, *, x/y/z
        normals_by_face_flat = tf.reshape(
            tf.transpose(normals_by_face, range(1, normals_by_face.get_shape(
            ).ndims - 1) + [0, normals_by_face.get_shape().ndims - 1]),
            [-1, face_count, 3]
        ...
        normals_by_vertex_flat = (_static_map_fn if static else tf.map_fn)(lambda normals_for_iib: tf.scatter_nd(
            ...
            updates=tf.reshape(
                tf.tile(normals_for_iib[:, tf.newaxis, :], [1, 3, 1]), [-1, 3]),
            shape=tf.shape(vertices)[-2:]
        ), normals_by_face_flat)
        normals_by_vertex = tf.reshape(
            normals_by_vertex_flat, tf.shape(vertices))
...
def split_vertices_by_face(vertices, faces, name=None):
    ...
    with ops.name_scope(name, 'SplitVerticesByFace', [vertices, faces]) as scope:
        ...
        flat_vertices = tf.reshape(
            vertices, [-1, vertices_shape[-2], vertices_shape[-1]])
        new_flat_vertices = tf.map_fn(lambda vertices_for_iib: tf.gather(
            vertices_for_iib, faces), flat_vertices)
        new_vertices = tf.reshape(new_flat_vertices, tf.concat(
            [vertices_shape[:-2], [face_count * 3, vertices_shape[-1]]], axis=0))

        new_faces = tf.reshape(tf.range(face_count * 3), [-1, 3])

        static_face_count = faces.get_shape(
        ).dims[0] if faces.get_shape().dims is not None else None
        static_new_vertex_count = static_face_count * \
            3 if static_face_count is not None else None
        if vertices.get_shape().dims is not None:
            new_vertices.set_shape(vertices.get_shape(
            ).dims[:-2] + [static_new_vertex_count] + vertices.get_shape().dims[-1:])
...
def diffuse_directional(vertex_normals, vertex_colors, light_direction, light_color, double_sided=True, name=None):
    ...
    with ops.name_scope(name, 'DiffuseDirectionalLight', [vertex_normals, vertex_colors, light_direction, light_color]) as scope:
        ...
        vertex_normals = tf.convert_to_tensor(
            vertex_normals, name='vertex_normals')
        vertex_colors = tf.convert_to_tensor(
            vertex_colors, name='vertex_colors')
        light_direction = tf.convert_to_tensor(
            light_direction, name='light_direction')
        light_color = tf.convert_to_tensor(light_color, name='light_color')

        # indexed by *, vertex-index, singleton
        cosines = tf.matmul(vertex_normals, -light_direction[..., tf.newaxis])
        if double_sided:
...
def specular_directional(vertex_positions, vertex_normals, vertex_reflectivities, light_direction, light_color, camera_position, shininess, double_sided=True, name=None):
    ...
    with ops.name_scope(name, 'SpecularDirectionalLight', [vertex_positions, vertex_normals, vertex_reflectivities, light_direction, light_color, camera_position, shininess]) as scope:
        ...
        vertex_positions = tf.convert_to_tensor(
            vertex_positions, name='vertex_positions')
        vertex_normals = tf.convert_to_tensor(
            vertex_normals, name='vertex_normals')
        vertex_reflectivities = tf.convert_to_tensor(
            vertex_reflectivities, name='vertex_reflectivities')
        light_direction = tf.convert_to_tensor(
            light_direction, name='light_direction')
        light_color = tf.convert_to_tensor(light_color, name='light_color')
        camera_position = tf.convert_to_tensor(
        ...
        reflected_directions = -vertices_to_light_direction + 2. * \
            tf.matmul(
                vertex_normals, vertices_to_light_direction[..., tf.newaxis]) * vertex_normals  # indexed by *, vertex-index, x/y/z
        # indexed by *, vertex-index, x/y/z
        vertex_to_camera_displacements = camera_position[...,
                                                         tf.newaxis, :] - vertex_positions
        cosines = tf.reduce_sum(
            (vertex_to_camera_displacements / tf.norm(vertex_to_camera_displacements,
                                                      axis=-1, keepdims=True) + 1.e-12) * reflected_directions,
...
def diffuse_point(vertex_positions, vertex_normals, vertex_colors, light_position, light_color, double_sided=True, name=None):
    ...
    with ops.name_scope(name, 'DiffusePointLight', [vertex_positions, vertex_normals, vertex_colors, light_position, light_color]) as scope:
        ...
        vertex_colors = tf.convert_to_tensor(
            vertex_colors, name='vertex_colors')
        light_position = tf.convert_to_tensor(
            light_position, name='light_position')
        light_color = tf.convert_to_tensor(light_color, name='light_color')

        relative_positions = vertex_positions - \
            light_position[..., tf.newaxis,
                           :]  # indexed by *, vertex-index, x/y/z
        incident_directions = relative_positions / \
            (tf.norm(relative_positions, axis=-1, keepdims=True) + 1.e-12)  # ditto
        # indexed by *, vertex-index
        cosines = tf.reduce_sum(vertex_normals * incident_directions, axis=-1)
        if double_sided:
            cosines = tf.abs(cosines)
        else:
            cosines = tf.maximum(cosines, 0.)
