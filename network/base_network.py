...
def reprojection(ytrue, ypred):
    ...
    projection_matrix = perspective_projection(
        FOCAL_LENGTH, CAMERA_CENTER, IMG_SIZE, IMG_SIZE, .1, 10)
    projection_matrix = tf.tile(tf.expand_dims(
        projection_matrix, 0), (b_size, 1, 1))
...
class GarmentNet(tf.keras.Model):
    def __init__(self, no_comp, garment_key, garmparams_sz):
        ...
        self.pca_comp = Dense(garmparams_sz, kernel_initializer=initializers.RandomNormal(
            0, 0.0005), name='pca_comp')
        self.PCA_ = PCA_(pca.components_.astype(
            'float32'), pca.mean_.astype('float32'))
...
class SingleImageNet(tf.keras.Model):
    def __init__(self, latent_code_garms_sz, latent_code_betas_sz, name=None):
        ...
        if IMG_SIZE >= 1000:
            self.conv1 = Conv2D(
                8, (3, 3), kernel_initializer='he_normal', padding='same', activation='relu')
    ...
    def append_coord(self, x):
        ...
        a = tf.tile(tf.expand_dims(
            tf.stack([a, b], axis=-1), 0), [K.int_shape(x)[0], 1, 1, 1])
        x = self.concat([x, a])
        return x
...
class PoseShapeOffsetModel(BaseModel):
    ...
    def __init__(self, config, latent_code_garms_sz=1024, garmparams_sz=config.PCA_, name=None):
        ...
        self.smpl_J = SmplBody25Layer(
            theta_in_rodrigues=False, theta_is_perfect_rotmtx=False, isHres=True)
        ...
        self.lat_betas = Dense(self.latent_code_betas_sz, kernel_initializer=initializers.RandomNormal(0, 0.00005),
                               activation='relu')
        self.betas = Dense(10, kernel_initializer=initializers.RandomNormal(
            0, 0.000005), name='betas')

        init_trans = np.array([0, 0.2, -2.])
        init_pose = np.load('assets/mean_a_pose.npy')
        init_pose[:3] = 0
        init_pose = tf.reshape(batch_rodrigues(
            init_pose.reshape(-1, 3).astype(np.float32)), (-1,))
        ...
        self.trans_layers = [NameLayer('trans_{}'.format(i))
                             for i in range(NUM)]
    ...
    def call(self, inp):
        ...
        out_dict = {}
        images = [tf.Variable(x, dtype=tf.float32, trainable=False)
                  for x in images]
        vertexlabel = tf.cast(tf.Variable(
            vertexlabel, trainable=False), tf.int32)
        if FACE:
            Js = [Lambda(lambda j: j[:, :25])(J) for J in Js_in]
        else:
            Js = [self.flatten(
                tf.cast(tf.Variable(x, trainable=False), tf.float32)) for x in Js_in]
        ...
        with tf.device('/gpu:0'):
            ...
            latent_code_pose = [tf.concat([q[1], x], axis=-1)
                                for q, x in zip(lat_codes, Js)]
        ...
        with tf.device('/gpu:0'):
            ...
            pose_trans_init = tf.tile(tf.expand_dims(
                self.pose_trans, 0), (K.int_shape(betas)[0], 1))

            poses_ = [self.lat_pose_layer(
                x) + pose_trans_init for x in latent_code_pose]
            ...
            garm_model_outputs = [fe(latent_code_offset_ShapeMerged)
                                  for fe in self.garmentModels]
            ...
            garment_verts_all_scattered = tf.concat(
                [K.expand_dims(
                    vertices_naked_, -1), tf.cast(garment_verts_all_scattered, vertices_naked_.dtype)],
                axis=-1)
            garment_verts_all_scattered = tf.transpose(
                garment_verts_all_scattered, perm=[0, 1, 3, 2])
            clothed_verts = tf.batch_gather(
                garment_verts_all_scattered, vertexlabel)
            clothed_verts = tf.squeeze(tf.transpose(
                clothed_verts, perm=[0, 1, 3, 2]))
            ...
            Js = [jl(self.smpl_J([p, betas, t]))
                  for jl, p, t in zip(self.J_layers, poses, trans)]
            vertices = tf.concat([tf.expand_dims(smpl, axis=-1)
                                  for i, smpl in enumerate(smpls)], axis=-1)
            ...
            vert_cols = tf.reshape(tf.gather(self.colormap, tf.reshape(
                vertexlabel, (-1,))), (-1, config.NVERTS, 3))
        ...
        renderered_garms_all = tf.transpose(
            renderered_garms_all, [1, 2, 3, 4, 0])
        out_dict['rendered'] = renderered_garms_all

        lap = compute_laplacian_diff(
            vertices_tposed, vertices_naked, self.faces)
        ##
    ...
    def loss_model(gt_dict, pred_dict, wt_dict):
        ...
        for k in wt_dict:
            if k == 'pca_verts':
                ...
                loss[k] = tf.losses.absolute_difference(
                    gt_dict[k], pred_dict[k], weights=wt_dict[k])

        return loss

    def train(self, inp_dict, gt_dict, loss_dict, vars2opt=None):
        images = [inp_dict['image_{}'.format(i)].astype(
            'float32') for i in range(NUM)]
        vertex_label = inp_dict['vertexlabel'].astype('int64')
        J_2d = [inp_dict['J_2d_{}'.format(i)].astype(
            'float32') for i in range(NUM)]
