...
class BaseModel(tf.keras.Model):
    def __init__(self, name = None):
        ...
        vert_file = open('assets/vert_spread.pkl', 'rb')
        u = pkl._Unpickler(vert_file)
        u.encoding = 'latin1'
        self.vertSpread = u.load()
        self.garmentModels = []
    ...
    def loss_model(gt_dict, pred_dict, wt_dict):
        ...
        for k in wt_dict:
            if k == 'pca_verts':
                ...
                loss[k] = tf.compat.v1.losses.absolute_difference(gt_dict[k], pred_dict[k], weights=wt_dict[k])
