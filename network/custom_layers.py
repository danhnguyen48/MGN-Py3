...
class Scatter_(Layer):
    def __init__(self, vs, NVERTS, **kwargs):
        ...
        self.vs = vs
        q = coo_matrix((np.ones((len(vs),)), (vs, range(len(vs)))),
                       shape=(NVERTS, len(vs)), dtype=np.float32)
    ...
    def build(self, input_shape):
        ...
        super(Scatter_, self).build(input_shape)

    def call(self, x):
        def fn(a):
            return tf.sparse.sparse_dense_matmul(self.vs, a)
